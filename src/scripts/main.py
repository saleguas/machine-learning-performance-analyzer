...
def main():
    ...
    if args['predict']:
        ...
        modelCreation.createPredictionProject(paths, int(args['predict'][0]) + modelCreation.ROLLING_WINDOW)

    if args['analyze']:
        paths = getFilePaths(location)
        modelCreation.createAnalyzeProject(paths, int(args['analyze'][0]) + modelCreation.ROLLING_WINDOW)
